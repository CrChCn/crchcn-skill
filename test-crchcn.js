// test-crchcn.js - Тест Merkle Tree для CRCHCN токена
const token = require('./crchcn-token');
const crypto = require('./working.js');

console.log('\n🦞=== ТЕСТ MERKLE TREE ДЛЯ CRCHCN ===🦞\n');
...
// Информация о токене
console.log('\nИнформация о токене CRCHCN:');
const info = token.info();
console.log(`  Символ: ${info.symbol}`);
console.log(`  Макс. предложение: ${info.maxSupply}`);
console.log(`  Лимит майнинга: ${info.mintLimit}`);

// Майним токены для пользователей
console.log('\nМайним токены CRCHCN...');
...
console.log('\nГенерируем Merkle Tree для CRCHCN...');
...
console.log('Переводим 300 CRCHCN от Alice к Bob...');
